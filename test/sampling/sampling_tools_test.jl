@testset "Sampling Tests" begin
    ...
    samples = hit_and_run(100_000, model, optimizer; keepevery=10, samplesize=5000, constraints=cons, solver_attributes=atts)
    @test isapprox(mean(samples[64,:]), 8.9, atol=0.5) # only tests if the sampler approximately converged
    
    samples = achr(100_000, model, optimizer; keepevery=10, samplesize=5000, constraints=cons)
    @test isapprox(mean(samples[64,:]), 8.9, atol=0.5) # only tests if the sampler approximately converged
