@testset "Reaction" begin
    ...
    rxns_d = OrderedDict(zip([r.id for r in rxns], rxns))
    dup, ind = check_duplicate_annotations(r4, rxns_d)
    @test dup && ind == "r1"

    dup, ind = check_duplicate_reaction(r4, rxns_d)
    @test dup && ind == "r3"

    dup, ind = check_duplicate_annotations(r2, rxns_d)
    @test !dup && ind == ""

    r5 = Reaction("r5", Dict(m3.id => -11.0, m4.id => 1.0), :bidirectional)
    dup, ind = check_duplicate_reaction(r5, rxns_d)
    @test !dup && ind == ""
end
