@testset "StandardModel tests" begin
    ...
    model = StandardModel()
    model.id = "model"
    model.reactions = OrderedDict(zip([r.id for r in rxns], rxns))
    model.metabolites = OrderedDict(zip([m.id for m in mets], mets))
    model.genes = OrderedDict(zip([g.id for g in gene_list], gene_list))
