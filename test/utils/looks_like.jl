@testset "Looks like in MatrixModel, detailed test" begin
    cp = test_LP()
    @test isempty(filter(looks_like_exchange_reaction, variable_ids(cp)))
    ...
    @test filter(looks_like_exchange_reaction, variable_ids(cp)) == ["EX_m1"]
    ...
    @test filter(looks_like_exchange_reaction, variable_ids(cp)) ==
          ["EX_m1", "Exch_m3", "Ex_m2"]
    @test filter(
