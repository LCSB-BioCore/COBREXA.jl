@testset "Coupling constraints" begin
    ...
    cp = test_coupledLP()
    changeCouplingBounds!(cp, [3, 1], cl=[-10., -20], cu=[10., 20])
    @test cp.cl[[1, 3]] == [-20, -10]
    @test cp.cu[[1, 3]] == [ 20,  10]
    changeCouplingBounds!(cp, [1000, 1001], cl=[-50., -60.])
    @test cp.cl[[1000, 1001]] == [-50., -60.]
end
