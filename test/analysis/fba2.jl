@testset "Flux balance analysis with CobraModel" begin
    ...
    sol = fba(model, optimizer; objective=biomass, constraints = cons)
    pfl = findfirst(model.reactions, "PFL")
    solmulti = fba(model, optimizer; objective=[biomass, pfl], weights = [0.8, 0.2]) # classic flux balance analysis
    ...
    sol = fba(model, optimizer; objective=biomass)
    ...
    cbmodel, v = makeOptimizationModel(model, optimizer, sense=sense)
    ubs = all_constraints(cbm, VariableRef, MOI.GreaterThan{Float64})
    lbs = all_constraints(cbm, VariableRef, MOI.LessThan{Float64})
