...
function _FVA_add_constraint(model, c, x, Z)
    Z[1] > -Inf && COBREXA.JuMP.@constraint(model, c' * x >= Z[1])
    Z[2] < Inf && COBREXA.JuMP.@constraint(model, c' * x <= Z[2])
end
...
function _FVA_optimize_reaction(model, rid, ret)
    ...
    if COBREXA.JuMP.termination_status(model) in [MOI.OPTIMAL, MOI.LOCALLY_SOLVED]
        return ret(model)
    else
        return nothing
    end
