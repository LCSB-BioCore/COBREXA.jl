...
"""
macro _is_reaction_fn(anno_id, identifier)
...
body = quote
    begin
        ...
        for key in annotation_keys
            if haskey(anno, key)
                any(in.(Identifiers.$(Symbol(identifier)), Ref(anno[key]))) && return true
            end
        end
        return false
    end
...
`Identifiers.$(identifier)` internally, which includes SBO identifiers. In
...
Expr(
    ...
    __source__,
    docstring,
    :($fname(model::MetabolicModel, reaction_id::String; annotation_keys = ["sbo", "SBO"]) = $body),
)
end