...
function check_duplicate_annotations(mets::Array{Metabolite, 1}, cmet::Metabolite)
    inspect_annotations = ["kegg.compound", "bigg.metabolite", "chebi", "inchi_key", "sabiork", "hmdb", "seed.compound", "metanetx.chemical", "reactome.compound", "biocyc"]
    catoms = get_atoms(cmet)
    for met in mets
        for anno in inspect_annotations
            if any([x in get(cmet.annotation, anno, ["c1"]) for x in get(met.annotation, anno, ["c2"])])
...
function get_atoms(met::Metabolite)
    ...
    for m in eachmatch(r"([A-Z]{1})([a-z]?)(\d*)", met.formula)
        ...
        atoms[string(element.match)] = isnothing(number) ? 1 : parse(Int64, string(number.match))
    end
    return atoms
end