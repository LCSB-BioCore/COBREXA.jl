...
function check_duplicate_annotations(genes::Array{Gene, 1}, cgene::Gene)
    inspect_annotations = ["ncbigene", "ncbigi", "refseq_locus_tag", "refseq_name", "refseq_synonym", "uniprot"]
    for gene in genes
        for anno in inspect_annotations
            if any([x in get(cgene.annotation, anno, ["c1"]) for x in get(gene.annotation, anno, ["c2"])])
