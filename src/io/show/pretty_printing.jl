...
"""
function _pretty_print(io, def::String, payload::String; def_color=:blue, payload_color=:magenta)
...
function _pretty_print(io, def::String, payload; def_color=:blue, payload_color=:magenta)
    print(io, Crayon(foreground=def_color), def)
    if isempty(payload)
        println(io, Crayon(foreground=payload_color), "---")
    else
