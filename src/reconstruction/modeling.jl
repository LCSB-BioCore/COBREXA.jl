...
function addReactions(m1::LinearModel,
    ...
    return addReactions(m1, m2.S, m2.b, m2.c, m2.xl, m2.xu, m2.rxns, m2.mets, checkConsistency=checkConsistency)
end
...
function addReactions(m::LinearModel,
                      ...
                      rxns::K,
                      mets::K;
                      checkConsistency=false) where {M<:MT,V<:VT,K<:ST}
    ...
    if checkConsistency
        (newReactions1, newMetabolites1) = verifyConsistency(m, Sp, b, c, xl, xu, rxns, mets, newReactions, newMetabolites)
    end
...
"""
function checkInputDimensions(S::M1,
                              b::V,
                              C::M2,
                              cl::V,
                              cu::V,
                              ...
                              xu::V,
                              rxns::K,
                              mets::K) where {M1<:MT,M2<:MT,V<:VT,K<:ST}
    n_c = length(c)

    length(cu) == length(cl) || throw(DimensionMismatch("`cl` and `cu` don't have the same size"))
    size(C) == (length(cu), n_c) || throw(DimensionMismatch("`C` shape doesn't match with `cu` and `c`"))

    checkInputDimensions(S, b, c, xl, xu, rxns, mets)

end

function checkInputDimensions(S::M,
                              b::V,
                              c::V,
                              xl::V,
                              xu::V,
                              rxns::K,
                              mets::K) where {M<:MT,V<:VT,K<:ST}

    n_c = length(c)
    n_b = length(b)

    length(xu) == length(xl) || throw(DimensionMismatch("`xl` and `xu` don't have the same size"))
    n_c == length(xl) || throw(DimensionMismatch("`c` doesn't have the same size as `xl`"))

    size(S) == (n_b, n_c) || throw(DimensionMismatch("`S` shape doesn't match with `c` and `mets`"))
...
"""
function verifyConsistency(m::LinearModel,
                     Sp::M,
                     b::V,
                     c::V,
                     xl::V,
                     ...
                     mets::K,
                     newReactions,
                     newMetabolites) where {M<:MT,V<:VT,K<:ST}
    ...
    if !isempty(newReactions)
        ...
        for (i, name) in enumerate(names)
            ...
            reaction = Sp[:, i]
            stoichIndex = findfirst(Bool[reaction == m.S[:, j] for j in 1:size(m.S, 2)])
...
"""
function removeReactions(m::LinearModel,
                         rxns::Array{Int64, 1})
    ...
    newModel = LinearModel(newS, newb, newC,
                           m.cl, m.cu, newc,
                           newxl, newxu, newRxns, newMets)
    return newModel
end
...
"""
function findExchangeReactions(model::LinearModel; excludeBiomass=false,
...
"""
function findExchangeMetabolites(model::LinearModel; excludeBiomass=false,
                                    biomassStr::String="biomass",
                                    excPrefs=["EX_"; "Exch_"; "Ex_"])
    excRxnInds = findExchangeReactions(model, excludeBiomass=excludeBiomass, biomassStr=biomassStr, excPrefs=excPrefs)
...
"""
function changeBounds!(model::LinearModel, rxns::Array{Int64, 1};
                       xl::V=Array{Float64}(undef, 0),
                       xu::V=Array{Float64}(undef, 0)) where {V<:VT}
    found = [index ∈ 1:nReactions(model) for index in rxns]
    length(rxns[found]) == length(unique(rxns[found])) || error("`rxns` appears to contain duplicates")

    if !isempty(xl)
        length(rxns) == length(xl) || throw(DimensionMismatch("`rxns` size doesn't match with `xl`"))
        model.xl[rxns[found]] = xl[found]
    end
    if !isempty(xu)
        length(rxns) == length(xu) || throw(DimensionMismatch("`rxns` size doesn't match with `xu`"))
        model.xu[rxns[found]] = xu[found]
    end
end


function changeBounds!(model::LinearModel, rxns::Array{String, 1};
                       xl::V=Array{Float64}(undef, 0),
                       xu::V=Array{Float64}(undef, 0)) where {V<:VT}
